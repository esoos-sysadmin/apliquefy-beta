...
const emptyForm: FormData = {
    title: "",
    personalInfo: {
        name: "", jobTitle: "", email: "", contact: "",
        address: "", desiredSalary: "", seniority: "",
        linkedinUrl: "", portfolio: "", github: "", professionalSummary: "",
    },
    experience: [],
    skills: [],
    education: [],
...
const emptyExperience: Experience = {
    companyName: "", jobType: "", description: "",
    jobArea: "", jobStartDate: "", jobEndDate: "", isActualJob: false,
};

const emptyEducation: Education = {
    nameOfInstitution: "", nameOfGraduation: "",
    StartDateOfGraduation: "", EndDateOfGraduation: "",
};
    ...
    useEffect(() => {
        async function fetchResume() {
            try {
                ...
                if (!res.ok) {
                    setPopup({ type: "error", message: json.message ?? "Erro ao carregar currículo" });
                    return;
                }

                const resume = json.data?.data ?? json.data;
                ...
                setForm({
                    ...
                    personalInfo: {
                        ...
                        contact: resume.personalInfo?.contact ?? "",
                        ...
                        professionalSummary: resume.personalInfo?.professionalSummary ?? "",
                    },
                    experience: resume.experience ?? [],
                    skills: resume.skills ?? [],
                    education: resume.education ?? [],
                    idioms: resume.idioms ?? [],
                });
            } catch {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!res.ok || !data.message) {
                setPopup({ type: "error", message: data.message ?? "Erro ao atualizar currículo" });
            } else {
                setIsEditing(false);
        ...
        <div style={{ maxWidth: 1152, margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                    ...
                    <p style={{ color: "#64748b", marginTop: 6, fontSize: 15 }}>
                        {isEditing
                            ? "Modo edição ativo — altere os campos e salve."
                            : "Clique em qualquer campo para começar a editar."}
                    </p>
                </div>
            ...
            <section style={sectionStyle}>
                ...
                <div style={gridTwo}>
                    <div>
                        ...
                        <input value={form.personalInfo.name} onChange={e => setPersonal("name", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(!!errors.name, !isEditing)} placeholder="Seu nome completo" />
                        {errors.name && <span style={errorStyle}>{errors.name}</span>}
                    </div>
                    <div>
                        <label style={labelStyle}>Professional Headline *</label>
                        <input value={form.personalInfo.jobTitle} onChange={e => setPersonal("jobTitle", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(!!errors.jobTitle, !isEditing)} placeholder="Ex: Senior Frontend Engineer" />
                        {errors.jobTitle && <span style={errorStyle}>{errors.jobTitle}</span>}
                    </div>
                    <div>
                        <label style={labelStyle}>Email Address *</label>
                        <input type="email" value={form.personalInfo.email} onChange={e => setPersonal("email", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(!!errors.email, !isEditing)} placeholder="seu@email.com" />
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onClick={unlock}
                            readOnly={!isEditing}
                            style={inputStyle(!!errors.contact, !isEditing)}
                    ...
                    <div style={{ gridColumn: "1 / -1" }}>
                        ...
                        <input value={form.personalInfo.address} onChange={e => setPersonal("address", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Rua, número, cidade - Estado" />
                    </div>
                    <div>
                        <label style={labelStyle}>Salário Desejado</label>
                        <input value={form.personalInfo.desiredSalary} onChange={e => setPersonal("desiredSalary", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Ex: R$ 8.000 / mês" />
                    </div>
                    <div>
                        <label style={labelStyle}>Nível de Senioridade atual</label>
                        <select
                            value={form.personalInfo.seniority}
                            onChange={e => setPersonal("seniority", e.target.value)}
                            onClick={unlock}
                            disabled={!isEditing}
                    ...
                    <div>
                        ...
                        <input value={form.personalInfo.linkedinUrl} onChange={e => setPersonal("linkedinUrl", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="linkedin.com/in/seuperfil" />
                    </div>
                    <div>
                        <label style={labelStyle}>Portfolio URL</label>
                        <input value={form.personalInfo.portfolio} onChange={e => setPersonal("portfolio", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="https://seusite.com" />
                    </div>
                    <div style={{ gridColumn: "1 / -1" }}>
                        ...
                        <textarea
                            ...
                            maxLength={500}
                            rows={4}
                            style={{ ...inputStyle(false, !isEditing), resize: "vertical" }}
                    ...
                    <div key={i} style={{ background: "#1C2333", borderRadius: 8, padding: 16, marginBottom: 12 }}>
                        ...
                        <div style={gridTwo}>
                            <div>
                                ...
                                <input value={exp.companyName} onChange={e => setExp(i, "companyName", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Nome da empresa" />
                            </div>
                            <div>
                                <label style={labelStyle}>Área de atuação</label>
                                <input value={exp.jobArea} onChange={e => setExp(i, "jobArea", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Ex: Tecnologia" />
                            </div>
                            <div>
                                <label style={labelStyle}>Tipo de contrato</label>
                                <input value={exp.jobType} onChange={e => setExp(i, "jobType", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Ex: CLT, PJ" />
                            </div>
                            <div>
                                <label style={labelStyle}>Data de início</label>
                                <input type={isEditing ? "month" : "text"} value={exp.jobStartDate} onChange={e => setExp(i, "jobStartDate", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} />
                            </div>
                            <div>
                                <label style={labelStyle}>Data de término</label>
                                <input type={isEditing ? "month" : "text"} value={exp.jobEndDate} onChange={e => setExp(i, "jobEndDate", e.target.value)} onClick={unlock} readOnly={!isEditing} disabled={exp.isActualJob} style={{ ...inputStyle(false, !isEditing), opacity: exp.isActualJob ? 0.4 : 1 }} />
                            </div>
                            {isEditing && (
                            ...
                            <div style={{ gridColumn: "1 / -1" }}>
                                ...
                                <textarea value={exp.description} onChange={e => setExp(i, "description", e.target.value)} onClick={unlock} readOnly={!isEditing} rows={3} style={{ ...inputStyle(false, !isEditing), resize: "vertical" }} placeholder="Descreva suas principais atividades..." />
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <section style={sectionStyle}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    ...
                    {form.skills.length === 0 && <p style={{ color: "#555", fontSize: 14 }}>Nenhuma skill adicionada.</p>}
                </div>
            </section>
                    ...
                    <div key={i} style={{ background: "#1C2333", borderRadius: 8, padding: 16, marginBottom: 12 }}>
                        ...
                        <div style={gridTwo}>
                            <div>
                                ...
                                <input value={edu.nameOfInstitution} onChange={e => setEdu(i, "nameOfInstitution", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Nome da instituição" />
                            </div>
                            <div>
                                <label style={labelStyle}>Curso</label>
                                <input value={edu.nameOfGraduation} onChange={e => setEdu(i, "nameOfGraduation", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Nome do curso" />
                            </div>
                            <div>
                                <label style={labelStyle}>Data de início</label>
                                <input type={isEditing ? "month" : "text"} value={edu.StartDateOfGraduation} onChange={e => setEdu(i, "StartDateOfGraduation", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} />
                            </div>
                            <div>
                                <label style={labelStyle}>Data de conclusão</label>
                                <input type={isEditing ? "month" : "text"} value={edu.EndDateOfGraduation} onChange={e => setEdu(i, "EndDateOfGraduation", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} />
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <section style={sectionStyle}>
                    ...
                    <div key={i} style={{ background: "#1C2333", borderRadius: 8, padding: 16, marginBottom: 12 }}>
                        ...
                        <div style={gridTwo}>
                            <div>
                                ...
                                <input value={idiom.language} onChange={e => setIdiom(i, "language", e.target.value)} onClick={unlock} readOnly={!isEditing} style={inputStyle(false, !isEditing)} placeholder="Ex: Inglês" />
                            </div>
                            <div>
                                <label style={labelStyle}>Nível</label>
                                <select
                                    value={idiom.level}
                                    onChange={e => setIdiom(i, "level", e.target.value)}
                                    onClick={unlock}
                                    disabled={!isEditing}
                                    style={{ ...inputStyle(false, !isEditing), appearance: "none" }}
                                >
                ...
                {form.idioms.length === 0 && <p style={{ color: "#555", fontSize: 14, textAlign: "center", padding: "24px 0" }}>Nenhum idioma adicionado ainda.</p>}
            </section>
            ...
            {isEditing && (
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, paddingTop: 8, paddingBottom: 32 }}>
                    <button
                        ...
                        style={{
                            height: 44, background: "transparent", color: "#888",
                            border: "1px solid #2A3445", borderRadius: 12,
                            padding: "0 20px", fontWeight: 600, fontSize: 14, cursor: "pointer",
                        }}
                    >
                        Cancelar
                    </button>
...
const inputStyle = (hasError = false, frozen = false): React.CSSProperties => ({
    width: "100%",
